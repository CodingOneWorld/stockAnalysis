...
if __name__ == '__main__':
    ...
    for l in df.values[:]:
        ...
        if '5' in desc:
            mm_l.append([code, name, 'm5',k1, k2])
        if '10' in desc:
            mm_l.append([code, name, 'm10',k1, k2])
        if '20' in desc:
            mm_l.append([code, name, 'm20',k1, k2])
        if '30' in desc:
            mm_l.append([code, name, 'm30',k1, k2])

        df_effect=pd.DataFrame(mm_l,columns=['code','name','mean','k1','k2'])
        print(df_effect)
